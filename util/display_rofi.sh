#!/bin/zsh
#
# Setup outputs with xrandr as a backend and rofi as a frontend
#
...
if ! command -v cat &>/dev/null; then
  ...
  echo 'Ran xrandr-rofi without xrandr? You'\''re gonna have a bad time.' >&2
  exit 255
elif ! command -v rofi &>/dev/null; then
  echo 'Ran xrandr-rofi without rofi? You'\''re gonna have a bad time.' >&2
  exit 255
...
function () {
  typeset opt
  typeset output
  typeset help_msg="usage: xrandr-rofi [options]
...
################################################################################
# Uses rofi to select the position of a given output relative to an already
...
################################################################################
# Uses rofi to display the detected mode options for a given output and lets
...
################################################################################
# Uses rofi to select the rotation of a given output. Prints the selection in
...
################################################################################
# Uses rofi to select an option. Undisplayed 'exit' option or pressing 'ESC'
...
function menu_select() {
  ...
  if [[ ${#menu} == 1 ]]; then
    ...
    while [[ -z "${menu[(r)${selection}]}" ]]; do
      echo "${(F)menu}" | rofi -dmenu -p "${prompt}" | read selection
